...
def router_node(state: RouterAgentState) -> Dict:
    ...
    # Map category to next node using Command
    next_node_map = {
    ...
    # Return the Command with next node and updated state
    return Command(
        goto=next_node_map[category],
...
def research_agent_node(state: RouterAgentState) -> Dict:
    ...
    # Using Command to update state and go to END
    return Command(
        goto=END,
        update={
            "agent_response": agent_response,
            "messages": [AIMessage(content=agent_response, name="research_agent")]
        }
    )
...
def writing_agent_node(state: RouterAgentState) -> Dict:
    ...
    return Command(
        ...
        update={
            "agent_response": agent_response,
            "messages": [AIMessage(content=agent_response, name="writing_agent")]
        }
    )
...
def planning_agent_node(state: RouterAgentState) -> Dict:
    ...
    return Command(
        ...
        update={
            "agent_response": agent_response,
            "messages": [AIMessage(content=agent_response, name="planning_agent")]
        }
    )
...
def critique_agent_node(state: RouterAgentState) -> Dict:
    ...
    return Command(
        ...
        update={
            "agent_response": agent_response,
            "messages": [AIMessage(content=agent_response, name="critique_agent")]
        }
    )
...
def create_router_agent_workflow():
    ...
    # Note: We don't need to add edges from agents to END when using Command approach
    # Each agent node returns a Command that specifically directs to END
